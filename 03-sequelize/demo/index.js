...
server.post("/players", async (req, res) => {//creacion de nueva fila en tabla Players
...
server.get("/players", async (req, res) => {
  const { name } = req.query; //?name=textoAFiltrar asi se hace una query
  const condition = name ? { where: { firstName: name } } : {};
...
server.get("/players/group", async (req, res) => {
  ...
  const stats = await Player.findAll({
    attributes: [attributeGroup, [db.fn(fn, db.col(attributeFn)), fn]],
...
//https://www.students.soyhenry.com/classes/31?cohortId=13&videoOrdinal=1
//2:00:00